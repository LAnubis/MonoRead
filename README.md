...
MonoRead 是一款基于 .NET MAUI 构建的现代化跨平台阅读引擎，致力于提供沉浸式的本地与云端纯文本（TXT）阅读体验，有Gemini pro的参与。

- 🚀 支持本地多文件与 WebDAV（如坚果云）多线程批量导入，内置 TXT 章节拆解算法。
- 🎨 对标顶级阅读应用的莫兰迪色系与极简 UI，沉浸式底部导航，告别一切视觉干扰。
- ☁️ 内置完整 WebDAV 协议栈，实现跨设备的书籍与进度同步。
- 📝 原生的文本高亮与读书笔记，支持笔记统一管理与导出。
- 📊 阅读热力图”与阅读历程统计。
